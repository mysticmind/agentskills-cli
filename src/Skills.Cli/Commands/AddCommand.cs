...
public sealed class AddCommand : AsyncCommand<AddCommand.Settings>
{
    ...
    public AddCommand(IInstallService installer, ISourceResolver resolver, IAnsiConsole console, Skills.Hosting.CommandCancellation cancellation)
    {
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        ...
        using (var preview = await StageWithSpinner(settings.Source, resolveOptions, _cancellation.Token))
        {
            ...
            var previewSubpath = string.IsNullOrWhiteSpace(settings.Path)
                ? preview.Subpath
                : Skills.SkillModel.PathSafety.SanitizeSubpath(settings.Path.Trim().Replace('\\', '/'));
