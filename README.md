...
**A .NET CLI tool for installing AI agent skills.** Pull skills from **GitHub**, **NuGet**, **npm**, well-known endpoints, or local folders and drop them into Claude Code, Cursor, Codex, OpenCode, and any other agent that follows the open [Agent Skills spec](https://agentskills.io).

Runs as `dnx agentskills` on .NET 10 (no install step) or as a global tool on .NET 8 LTS. A .NET-native port of [vercel-labs/skills](https://github.com/vercel-labs/skills) that adds first-class NuGet and npm registry support.
...
Working samples live under [`samples/`](samples). Each is self-contained and exercised by the smoke-test instructions in its own README.

| Sample | What it shows | Try it |
|---|---|---|
| [`samples/hello-skill`](samples/hello-skill) | The minimal valid skill: one folder, one `SKILL.md` with required frontmatter. Used as the local-source smoke test throughout this README. | `agentskills add ./samples/hello-skill -a universal -y --copy` |
| [`samples/sample-nuget-package`](samples/sample-nuget-package) | A `.csproj` that packs one skill into the standard `contentFiles/any/any/skills/` NuGet layout. Includes pack + push instructions for a local file feed. | `dotnet pack samples/sample-nuget-package -o ./local-feed && agentskills add Sample.SkillPackage --nuget-source ./local-feed -a universal -y --copy` |
| [`samples/sample-npm-package`](samples/sample-npm-package) | A `package.json` plus `skills/<name>/SKILL.md` showing the npm-tarball layout (`package/skills/...` on the wire). Sample README covers Verdaccio-based local testing without publishing to npmjs.org. | `npm pack samples/sample-npm-package` (then publish to Verdaccio or npmjs.org and `agentskills add @acme/agent-skills-sample`) |

Each sample's own README has the step-by-step verification commands; this section just helps you find them.
