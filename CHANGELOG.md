...
The first public release of `skills-net` - a .NET port of [vercel-labs/skills](https://github.com/vercel-labs/skills) implementing the open [Agent Skills spec](https://agentskills.io).
...
- **Multi-targeted: `net8.0` and `net10.0`** - `dotnet tool install --global Skills` works on either runtime; `dnx skills` is .NET 10 only (because `dnx` itself ships with the .NET 10 SDK).
